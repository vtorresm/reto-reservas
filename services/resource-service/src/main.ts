...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('API para gestión de recursos y espacios de coworking')
    .setVersion('1.0')
    .addBearerAuth(
      {
        type: 'http',
  ...
  logger.log(`🚀 Resource Service iniciado en puerto ${port}`);
...
bootstrap().catch((error) => {
  console.error('❌ Error iniciando Resource Service:', error);
  process.exit(1);
});