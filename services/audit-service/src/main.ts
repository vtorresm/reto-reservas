...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('API para logging centralizado y auditoría')
    .setVersion('1.0')
    .addBearerAuth(
      {
        type: 'http',
  ...
  logger.log(`🚀 Audit Service iniciado en puerto ${port}`);
...
bootstrap().catch((error) => {
  console.error('❌ Error iniciando Audit Service:', error);
  process.exit(1);
});