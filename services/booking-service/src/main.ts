...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('API para gestión de reservas y calendarios')
    .setVersion('1.0')
    .addBearerAuth(
      {
        type: 'http',
  ...
  logger.log(`🚀 Booking Service iniciado en puerto ${port}`);
...
bootstrap().catch((error) => {
  console.error('❌ Error iniciando Booking Service:', error);
  process.exit(1);
});