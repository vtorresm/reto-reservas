...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('API para gestión de notificaciones y comunidad')
    .setVersion('1.0')
    .addBearerAuth(
      {
        type: 'http',
  ...
  logger.log(`🚀 Notification Service iniciado en puerto ${port}`);
...
bootstrap().catch((error) => {
  console.error('❌ Error iniciando Notification Service:', error);
  process.exit(1);
});